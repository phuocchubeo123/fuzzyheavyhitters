...
impl NaiveProtocol {
    ...
    pub fn run_server_unknown_dictionary_parallel(
        ...
        for prefix_length in 1..=max_bit_length*dimension {
            ...
            let new_data: Vec<Vec<Vec<u8>>> = self.unknown_dictionary_parallel_tree_expand(&current_data, &current_prefixes, client_shares, eval_modulus).unwrap();
            ...
            let count_shares: Vec<Mod2k> = new_eval
                .par_iter()
                .map(|evals| {
                    let mut acc = 0u128;
    ...
    fn unknown_dictionary_parallel_tree_expand(
        &self, 
        current_data: &[Vec<Vec<u8>>], 
        current_prefixes: &[Vec<bool>],
        client_shares: &[DpfKey],
        eval_modulus: u128,
